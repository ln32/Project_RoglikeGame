...
public class MapSettingDataSet : MonoBehaviour
{
   ...
   internal GameObject GetDataSet_byStage(int _value)
    ...
    public void sad()
    {
        for (int i = 0; i < mapSettingDataSet.Count; i++)
        {
            int target = i % bgPrefabList.Length;
            mapSettingDataSet[i].bgPrefabList = bgPrefabList[target];
        }
    }
}

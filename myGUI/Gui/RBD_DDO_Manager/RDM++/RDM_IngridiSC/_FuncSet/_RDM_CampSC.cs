...
using static _InvenSetManagerPrivateFunc;
...
public static class _RDM_CampSC
{
    internal static void SetIngredientByInventorySlot(this RDM_CampSC campSC, SlotGUI_InvenSlot src_Slot, RBD_IngridimentSlot dst_Slot)
    {
        ...
        int conflictIndex = campSC._GUI_IngridiSlotManager.Values.IsDisAvabibleValue(sourceItemGUI._myData.index);

        if (conflictIndex > -1)
        {
            campSC._GUI_IngridiSlotManager.Values.RBD_Slots[conflictIndex].SetDefault();
        }
    ...
    internal static void GetInventoryItems_byItemIndex(this RDM_CampSC campSC)
    {
        ...
        int[] ingredientIndexes = ingredientSlotManager.Values.GetCookSlotsItemIndex();

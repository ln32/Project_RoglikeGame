...
{
    internal static void SetIngredient_byInvenSlot(this RDM_Info_CampCook cookCamp, SlotGUI_InvenSlot sourceSlot, RBD_IngridimentSlot destinationSlot)
    {
        var itemData = SGT_GUI_ItemData.GetInstance();
        GUI_ItemUnit sourceItemGUI = sourceSlot._itemGUI;
        int crashIndex = cookCamp.GUI_IngridiSlotManager.Values.IsDisAvabibleValue(sourceItemGUI._myData.index);

        // Handle item crash
        if (crashIndex > -1)
        {
            cookCamp.GUI_IngridiSlotManager.Values.RBD_Slots[crashIndex].SetDefault();
    ...
    internal static void GetInvenItem_byItemIndex(this RDM_Info_CampCook cookCamp)
    {
        var ingredientSlotManager = cookCamp.GUI_IngridiSlotManager;

        // Check if ingredients are ready
        if (!ingredientSlotManager.Values.checkCurr())
        {
    ...
    private static int DetermineTargetLevel(GUI_IngridiSlotManager ingredientSlotManager)
    {
        ...
        List<Vector2Int> patternData = ingredientSlotManager.IngredientPoker.Check_PatternList();
        ingredientSlotManager.IngredientPoker.GetV3_byData(patternData, out stdV2_X, out stdV2_Y);
    ...
    private static void UpdateInventoryData(RDM_Info_CampCook cookCamp, GUI_IngridiSlotManager ingredientSlotManager)
    {
        int[] ingredientIndices = ingredientSlotManager.Values.GetCookSlotsItemIndex();
